...
**拍卖行**：玩家选背包物品发起拍卖（起拍价、每次最低加价、时长），全服播报；其他玩家点进拍卖页出价，出价为**固定档位按钮**——加价 1 次/5 次/10 次（每次 = 该拍卖的最低加价），不支持手动输金额。出价前先**验资**：总资产（余额 + 定期存款 + 领地价值）不够覆盖的档位会被标灰拦截。截止时按最高价强制收款。
...
- 出价为**固定档位**：基准价（当前最高价，无人出价时为起拍价）+ 1/5/10 次 × 最低加价，点档位后需二次确认；卖家不能竞拍自己的拍卖，当前最高出价者不能给自己抬价。
